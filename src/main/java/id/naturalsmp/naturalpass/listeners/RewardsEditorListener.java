...
import id.naturalsmp.naturalpass.gui.LevelRewardEditGui;
import id.naturalsmp.naturalpass.gui.RewardsCategoryGui;
import id.naturalsmp.naturalpass.gui.RewardsEditorGui;
...
public class RewardsEditorListener implements Listener {
    ...
    public RewardsEditorListener(NaturalPass plugin) {
        this.plugin = plugin;
    }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        } else if ((title.contains(FREE_REWARDS_KEYWORD) || title.contains(PREMIUM_REWARDS_KEYWORD))
                && title.contains("Page")) {
            event.setCancelled(true);
    ...
    private void handleRewardsCategoryClick(Player player, int slot, boolean isPremium) {
        ...
        if (slot < 45) {
            ...
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                plugin.getGuiManager().openNaturalPassGUI(player, 1);
            }, 20L);
        }
    }
