...
import id.naturalsmp.naturalpass.managers.MissionEditorManager;
...
public class MissionEditorListener implements Listener {
    ...
    public MissionEditorListener(NaturalPass plugin) {
        this.plugin = plugin;
    }
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        if (plugin.getMissionEditorManager().isEditing(event.getPlayer().getUniqueId())) {
            MissionEditorManager.EditorState state = plugin.getMissionEditorManager()
                    .getEditingState(event.getPlayer().getUniqueId());
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (title.startsWith("Mission Editor")) {
            ...
            if (!plugin.getConfigManager().getMissionsConfig().contains("mission-pools." + missionKey))
                return;
    ...
    private void handleEditorClick(Player player, InventoryClickEvent event, String title) {
        try {
            ...
            if (clicked.getType() == Material.ARROW) {
                if (event.getSlot() == 45)
                    plugin.getMissionEditorManager().openMissionEditor(player, page - 1);
                if (event.getSlot() == 53)
                    plugin.getMissionEditorManager().openMissionEditor(player, page + 1);
    ...
    private void handleTypeSelectionClick(Player player, InventoryClickEvent event) {
        ...
        if (clicked.getType() == Material.BARRIER) {
            if (plugin.getMissionEditorManager().isEditing(player.getUniqueId())) {
                MissionEditorManager.EditorState state = plugin.getMissionEditorManager()
                        .getEditingState(player.getUniqueId());
        ...
        ItemMeta meta = clicked.getItemMeta();
        if (meta != null && meta.getPersistentDataContainer().has(plugin.getCustomItemManager().getPremiumItemKey(),
                PersistentDataType.STRING)) {
            String type = meta.getPersistentDataContainer().get(plugin.getCustomItemManager().getPremiumItemKey(),
                    PersistentDataType.STRING);
    ...
    private void handleDetailsClick(Player player, InventoryClickEvent event, String key) {
        ...
        if (slot == 12) {
            ConfigurationSection section = plugin.getConfigManager().getMissionsConfig()
                    .getConfigurationSection("mission-pools." + key);
            if (section != null) {
        ...
        switch (slot) {
            case 10:
                type = MissionEditorManager.EditType.DISPLAY_NAME;
                break;
            case 11:
                type = MissionEditorManager.EditType.TYPE;
                break;
            case 12:
                if (isTargetRequired)
                    type = MissionEditorManager.EditType.TARGET;
                break;
            case 14:
                type = MissionEditorManager.EditType.MIN_REQ;
                break;
            case 15:
                type = MissionEditorManager.EditType.MAX_REQ;
                break;
            case 22:
                type = MissionEditorManager.EditType.WEIGHT;
                break;
            case 23:
                type = MissionEditorManager.EditType.MIN_XP;
                break;
            case 24:
                type = MissionEditorManager.EditType.MAX_XP;
                break;
            case 31:
                plugin.getMissionEditorManager().openMissionEditor(player, 1);
                return;
