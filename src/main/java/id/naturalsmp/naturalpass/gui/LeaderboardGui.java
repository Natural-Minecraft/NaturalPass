...
public class LeaderboardGui extends BaseGui {
    ...
    public LeaderboardGui(NaturalPass plugin, Player player) {
    ...
    private void setupTitleItem(Inventory gui) {
        ...
        for (String line : plugin.getMessageManager().getMessagesConfig()
                .getStringList("items.leaderboard-title.lore")) {
        ...
        plugin.getDatabaseManager().getTop10Players().thenAccept(topPlayers -> {
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                for (int i = 0; i < topPlayers.size() && i < 10; i++) {
                    ...
                    String rank;
                    if (i == 0)
                        rank = plugin.getMessageManager().getMessage("items.leaderboard-rank.first");
                    else if (i == 1)
                        rank = plugin.getMessageManager().getMessage("items.leaderboard-rank.second");
                    else if (i == 2)
                        rank = plugin.getMessageManager().getMessage("items.leaderboard-rank.third");
                    else
                        rank = plugin.getMessageManager().getMessage("items.leaderboard-rank.other", "%rank%",
                                String.valueOf(i + 1));
                    ...
                    List<String> lore = new ArrayList<>();
                    for (String line : plugin.getMessageManager().getMessagesConfig()
                            .getStringList("items.leaderboard-player.lore")) {
                        String processedLine = line
